    ...
    const NVM::FlashMemory::Physical_Page_Address& /*plane_address*/)
{
    ...
    for (unsigned int b = 0; b < pbk->Total_blocks; ++b) {
        ...
        double migration   = static_cast<double>(blk.Valid_page_count)   / total_pages_d;
...
void GC_and_WL_Unit_Page_Level_RRA::Update_adaptive_threshold(
    PlaneBookKeepingType* pbk)
{
    if (!pbk || pbk->Total_blocks == 0) return;

    double neutral = static_cast<double>(pbk->Blocks[0].Pages_no * pbk->Total_blocks);
    ...
    double avg_wear = (sum_ec / pbk->Total_blocks) / m_pe_endurance;
